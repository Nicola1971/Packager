...
if (isset($_REQUEST['Go'])) {
    ...
    // Process chunks
        ...
        $item->tags['modx_category'] = $cats[$chunk['category']];

        $item->code = $chunk['snippet'];

        $item->Package();
        $destDir = $packageDir . 'install/assets/' . $matches[1] ;
        echo $item->Write($destDir);
    ...
    // process Modules
        ...
        $item->tags['modx_category'] = $cats[$module['category']];
    ...
    while ($snippet = array_shift($process['snippets'])) {
        ...
        $item->tags['modx_category'] = $cats[$snippet['category']];
    ...
    while ($plugin = array_shift($process['plugins'])) {
        ...
        $item->tags['modx_category'] = $cats[$plugin['category']];
    ...
    while ($template = array_shift($process['templates'])) {
        ...
        $item->tags['modx_category'] = $cats[$template['category']];
    ...
    while ($tv = array_shift($process['tvs'])) {
    	...
    	$item->tags['modx_category']  = $cats[$tv['category']];
...
/* Copy relevant subdirectories */
    foreach ($filesFolders as $element=>$fileSets) {
        ...
        foreach ($fileSets as $fileSet) {
            ...
            $destDir = $packageDir . 'assets/' . $matches[1] ;
